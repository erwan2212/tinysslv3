...
uses
  ...
  opensslutils,
  utils,userverhttps,crt, uwinstore ;
...
begin
  ...
  cmd.declareflag('server', 'runs a https server using server.crt and server.key');

  cmd.declareflag('list_store', 'list certs in a store defined with ca');
  //
  cmd.parse(cmdline);
  ...
  begin
    ...
    if cmd.existsProperty('print_sshkey')=true then
    begin
    try
    LoadSSL;
    key:=cmd.readString('key');
    ...
    if cmd.existsProperty('server')=true then
    begin
    try
    LoadSSL;
    //
    // Pour démarrer le serveur sur le port par défaut (8443) ou un autre :
    StartHttpsServer(8443);

    // Pour l'arrêter manuellement ou écouter la touche ESC :
    // (Par exemple dans une boucle de votre choix)
    repeat
      if KeyPressed then
        if ReadKey = #27 then StopHttpsServer;
      Sleep(50);
    until not IsHttpsServerRunning;
    //
    finally
    FreeSSL;
    end;
    ...
    if cmd.existsProperty('list_store')=true then
    begin
    try
    LoadSSL;
    ca_val:=cmd.readString('ca');
    list_windows_store (ca_val);
    finally
    FreeSSL;
    end;
    exit;
