...
function Decrypt_Priv(ACryptedData: string;var decrypted:string): boolean;
...
function getDN2(pDn: pX509_NAME): String;
...
procedure FreeSSL;
begin
  // Sous OpenSSL 3.x, EVP_cleanup et ERR_free_strings n'existent plus.
  // La fonction globale de nettoyage moderne est OPENSSL_cleanup.

  // Note : Dans de nombreux cas avec OpenSSL 3, le nettoyage par le système
  // à la fermeture du processus suffit, mais si vous souhaitez un nettoyage explicite :
  ...
  try
    ...
    try
      ...
      try
        ...
        // Récupération directe et propre du contenu Base64 depuis le memory BIO
        b64len := BIO_get_mem_data(bio_mem, @memPtr);
        if (b64len > 0) and (memPtr <> nil) then
        begin
          SetLength(b64data, b64len);
          Move(memPtr^, b64data[1], b64len);
        ...
        // BIO_free_all libère toute la chaîne (b64 + bio_mem d'un seul coup)
...
begin
  ...
  try
    ...
    // SYNC OBLIGATOIRE AVEC Encrypt_Pub : Utilisation de OAEP
    ...
    outLen := 0;
    if EVP_PKEY_decrypt(dctx, nil, @outLen, binBuf, binLen) <> 1 then
    begin
      err := ERR_get_error;
      repeat
        log(string(ERR_error_string(err, nil)), 1);
    ...
    realOutLen := outLen; // Sécurité sur la taille allouée

    if EVP_PKEY_decrypt(dctx, outBuf, @realOutLen, binBuf, binLen) <> 1 then
    begin
      err := ERR_get_error;
      repeat
        log(string(ERR_error_string(err, nil)), 1);
    ...
    // =========================================================
    SetLength(plainText, realOutLen);
    if realOutLen > 0 then
      Move(outBuf^, plainText[1], realOutLen);
...
function getDN2(pDn: pX509_NAME): String;
// Remplacement dans getdn ou dans list_windows_store :
var bio: pBIO;
    len: integer;
    buf: AnsiString;
begin
  bio := BIO_new(BIO_s_mem());
  X509_NAME_print_ex(bio, pDn, 0, XN_FLAG_RFC2253 or ASN1_STRFLGS_UTF8_CONVERT);
  len := BIO_pending(bio);
  SetLength(buf, len);
  BIO_read(bio, @buf[1], len);
  BIO_free(bio);
  result := string(buf);
end;
