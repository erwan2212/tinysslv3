...
{
  Generated from OpenSSL 3.0.20 Header File stack.h - Tue 19 May 14:25:44 BST 2026
...
begin
  ...
  if not assigned(OPENSSL_sk_num) then
    ...
    OPENSSL_sk_num := @LEGACY_OPENSSL_sk_num;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_value) then
    ...
    OPENSSL_sk_value := @LEGACY_OPENSSL_sk_value;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_set) then
    ...
    OPENSSL_sk_set := @LEGACY_OPENSSL_sk_set;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_new) then
    ...
    OPENSSL_sk_new := @LEGACY_OPENSSL_sk_new;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_new_null) then
    ...
    OPENSSL_sk_new_null := @LEGACY_OPENSSL_sk_new_null;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_new_reserve) then
    ...
    OPENSSL_sk_new_reserve := @LEGACY_OPENSSL_sk_new_reserve;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_reserve) then
    ...
    OPENSSL_sk_reserve := @LEGACY_OPENSSL_sk_reserve;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_free) then
    ...
    OPENSSL_sk_free := @LEGACY_OPENSSL_sk_free;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_pop_free) then
    ...
    OPENSSL_sk_pop_free := @LEGACY_OPENSSL_sk_pop_free;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_deep_copy) then
    ...
    OPENSSL_sk_deep_copy := @LEGACY_OPENSSL_sk_deep_copy;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_insert) then
    ...
    OPENSSL_sk_insert := @LEGACY_OPENSSL_sk_insert;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_delete) then
    ...
    OPENSSL_sk_delete := @LEGACY_OPENSSL_sk_delete;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_delete_ptr) then
    ...
    OPENSSL_sk_delete_ptr := @LEGACY_OPENSSL_sk_delete_ptr;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_find) then
    ...
    OPENSSL_sk_find := @LEGACY_OPENSSL_sk_find;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_find_ex) then
    ...
    OPENSSL_sk_find_ex := @LEGACY_OPENSSL_sk_find_ex;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_find_all) then
    ...
    OPENSSL_sk_find_all := @LEGACY_OPENSSL_sk_find_all;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_push) then
    ...
    OPENSSL_sk_push := @LEGACY_OPENSSL_sk_push;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_unshift) then
    ...
    OPENSSL_sk_unshift := @LEGACY_OPENSSL_sk_unshift;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_shift) then
    ...
    OPENSSL_sk_shift := @LEGACY_OPENSSL_sk_shift;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_pop) then
    ...
    OPENSSL_sk_pop := @LEGACY_OPENSSL_sk_pop;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_zero) then
    ...
    OPENSSL_sk_zero := @LEGACY_OPENSSL_sk_zero;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_set_cmp_func) then
    ...
    OPENSSL_sk_set_cmp_func := @LEGACY_OPENSSL_sk_set_cmp_func;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_dup) then
    ...
    OPENSSL_sk_dup := @LEGACY_OPENSSL_sk_dup;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_sort) then
    ...
    OPENSSL_sk_sort := @LEGACY_OPENSSL_sk_sort;
    {$else}
...
begin
  ...
  if not assigned(OPENSSL_sk_is_sorted) then
    ...
    OPENSSL_sk_is_sorted := @LEGACY_OPENSSL_sk_is_sorted;
    {$else}
...
begin
  ...
  if not assigned(sk_value) then
    {$if declared(LEGACY_OPENSSL_sk_value)}
    sk_value := @LEGACY_OPENSSL_sk_value;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_value');
    {$ifend}
...
begin
  ...
  if not assigned(sk_new_null) then
    {$if declared(LEGACY_OPENSSL_sk_new_null)}
    sk_new_null := @LEGACY_OPENSSL_sk_new_null;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_new_null');
    {$ifend}
  Result := sk_new_null;
...
begin
  ...
  if not assigned(sk_free) then
    {$if declared(LEGACY_OPENSSL_sk_free)}
    sk_free := @LEGACY_OPENSSL_sk_free;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_free');
    {$ifend}
  sk_free(_param1);
...
begin
  ...
  if not assigned(sk_pop_free) then
    {$if declared(LEGACY_OPENSSL_sk_pop_free)}
    sk_pop_free := @LEGACY_OPENSSL_sk_pop_free;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_pop_free');
    {$ifend}
  sk_pop_free(st, func);
...
begin
  ...
  if not assigned(sk_deep_copy) then
    {$if declared(LEGACY_OPENSSL_sk_deep_copy)}
    sk_deep_copy := @LEGACY_OPENSSL_sk_deep_copy;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_deep_copy');
    {$ifend}
...
begin
  ...
  if not assigned(sk_insert) then
    {$if declared(LEGACY_OPENSSL_sk_insert)}
    sk_insert := @LEGACY_OPENSSL_sk_insert;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_insert');
    {$ifend}
...
begin
  ...
  if not assigned(sk_delete) then
    {$if declared(LEGACY_OPENSSL_sk_delete)}
    sk_delete := @LEGACY_OPENSSL_sk_delete;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_delete');
    {$ifend}
...
begin
  ...
  if not assigned(sk_delete_ptr) then
    {$if declared(LEGACY_OPENSSL_sk_delete_ptr)}
    sk_delete_ptr := @LEGACY_OPENSSL_sk_delete_ptr;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_delete_ptr');
    {$ifend}
...
begin
  ...
  if not assigned(sk_find) then
    {$if declared(LEGACY_OPENSSL_sk_find)}
    sk_find := @LEGACY_OPENSSL_sk_find;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_find');
    {$ifend}
...
begin
  ...
  if not assigned(sk_find_ex) then
    {$if declared(LEGACY_OPENSSL_sk_find_ex)}
    sk_find_ex := @LEGACY_OPENSSL_sk_find_ex;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_find_ex');
    {$ifend}
...
begin
  ...
  if not assigned(sk_push) then
    {$if declared(LEGACY_OPENSSL_sk_push)}
    sk_push := @LEGACY_OPENSSL_sk_push;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_push');
    {$ifend}
...
begin
  ...
  if not assigned(sk_unshift) then
    {$if declared(LEGACY_OPENSSL_sk_unshift)}
    sk_unshift := @LEGACY_OPENSSL_sk_unshift;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_unshift');
    {$ifend}
...
begin
  ...
  if not assigned(sk_shift) then
    {$if declared(LEGACY_OPENSSL_sk_shift)}
    sk_shift := @LEGACY_OPENSSL_sk_shift;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_shift');
    {$ifend}
  Result := sk_shift(st);
...
begin
  ...
  if not assigned(sk_zero) then
    {$if declared(LEGACY_OPENSSL_sk_zero)}
    sk_zero := @LEGACY_OPENSSL_sk_zero;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_zero');
    {$ifend}
  sk_zero(st);
...
begin
  ...
  if not assigned(sk_set_cmp_func) then
    {$if declared(LEGACY_OPENSSL_sk_set_cmp_func)}
    sk_set_cmp_func := @LEGACY_OPENSSL_sk_set_cmp_func;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_set_cmp_func');
    {$ifend}
...
begin
  ...
  if not assigned(sk_sort) then
    {$if declared(LEGACY_OPENSSL_sk_sort)}
    sk_sort := @LEGACY_OPENSSL_sk_sort;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_sort');
    {$ifend}
  sk_sort(st);
...
begin
  ...
  if not assigned(sk_is_sorted) then
    {$if declared(LEGACY_OPENSSL_sk_is_sorted)}
    sk_is_sorted := @LEGACY_OPENSSL_sk_is_sorted;
    {$else}
    EOpenSSLAPIFunctionNotPresent.RaiseException('OPENSSL_sk_is_sorted');
    {$ifend}
